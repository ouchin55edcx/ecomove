...
public class JourneyUi {
    ...
    private void printJourneyDetails(Journey journey) {
            ...
            System.out.printf("│ Departure: %-30s │\n", formatDateTime(journey.getDepartureTime(), dateFormatter, dateTimeFormatter));
            System.out.printf("│ Arrival: %-32s │\n", formatDateTime(journey.getArrival_time(), dateFormatter, dateTimeFormatter));
            System.out.println("├────────────────────────────────────────┤");
            ...
            if (tickets == null || tickets.isEmpty()) {
                ...
                for (Ticket ticket : tickets) {
                    ...
                    System.out.printf("│ Sale Date: %-29s │\n", formatDateTime(ticket.getSaleDate(), dateFormatter, dateTimeFormatter));
                }
            }
            System.out.println("└────────────────────────────────────────┘");
        }

    private String formatDateTime(Object dateTime, DateTimeFormatter dateFormatter, DateTimeFormatter dateTimeFormatter) {
        if (dateTime instanceof LocalDateTime) {
            return ((LocalDateTime) dateTime).format(dateTimeFormatter);
        } else if (dateTime instanceof LocalDate) {
            return ((LocalDate) dateTime).format(dateFormatter);
        } else {
            return "N/A";
        }
    }
